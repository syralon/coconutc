...
func RepositoryBuilder(pkg string) BuildFunc {
...
func (b *repositoryBuilder) structs() {
	...
	b.file.Type().Id(b.name).Struct(
		jen.Id("client").Op("*").Qual(b.EntPackage, b.node.Name+"Client"),
	)
	b.file.Func().Id("New"+b.name).Params(
		jen.Id("client").Op("*").Qual(b.EntPackage, "Client"),
	).Op("*").Id(b.name).Block(
		define("rep").Op("&").Id(b.name).Block(jen.Id("client").Op(":").Id("client").Op(".").Id(b.node.Name).Op(",")),
...
func (b *repositoryBuilder) fnGet() *jen.Statement {
	return jen.Id("Get").
		...
		Call(jen.Op("*").Qual(b.EntPackage, b.node.Name), jen.Error())
}
...
func (b *repositoryBuilder) fnList(paginatorName string) *jen.Statement {
	return jen.Id("List").
		...
		Call(
			jen.Index().Op("*").Qual(b.EntPackage, b.node.Name),
...
func (b *repositoryBuilder) fnEdges(edge *gen.Edge, paginatorName string) *jen.Statement {
	...
	return jen.Id(name).
		...
		Call(
			jen.Index().Op("*").Qual(b.EntPackage, edge.Type.Name),
			jen.Op("*").Qual(pkgCoconutField, paginatorName),
			jen.Error(),
		)
}
...
func (b *repositoryBuilder) fnEdge(edge *gen.Edge) *jen.Statement {
	...
	return jen.Id(name).
		...
		Call(jen.Op("*").Qual(b.EntPackage, edge.Type.Name), jen.Error())
}
...
func (b *repositoryBuilder) set() {
	for _, v := range b.node.Fields {
		...
		b.fn(b.fnSet(v)).
			Block(
				define("err").Id("rep").Dot("client").Dot("Update").Call().
...
func (b *repositoryBuilder) edges() {
	for _, edge := range b.node.Edges {
		...
		b.fn(b.fnEdges(edge, opts.PaginatorStyle.String())).Block(
			...
			jen.Line(),
			jen.Return(jen.Id("data"), jen.Id("paginator"), jen.Nil()),
		)
	}
}
	...
	for _, edge := range b.node.Edges {
		...
		b.fn(b.fnEdge(edge)).
			...
			Block(
				...
				jen.Return(jen.Id("query").Dot("First").Call(jen.Id("ctx"))),
			)
	}
}
...
func (b *repositoryBuilder) list() {
	...
	b.fn(b.fnList(paginatorName)).
		Block(
			define("query").Id("rep").Dot("client").Dot("Query()").Dot("Where").Call(
...
func (b *repositoryBuilder) create() {
	...
	b.fn(b.fnCreate()).
		Block(
			create,
			jen.Return(jen.Id("create").Dot("Save").Call(jen.Id("ctx"))),
		)
}
...
func (b *repositoryBuilder) update() {
	...
	b.fn(b.fnUpdate()).
		Block(
			define("update").Id("rep").Dot("client").Dot("UpdateOneID").Call(jen.Id("id")),
			jen.Add(fields...),
...
func (b *repositoryBuilder) delete() {
	...
	b.fn(b.fnDelete()).
		Block(
			jen.Return(jen.Id("rep").Dot("client").Dot("DeleteOneID").Call(jen.Id("id"))).Dot("Exec").Call(jen.Id("ctx")),
		)
}

func (b *repositoryBuilder) queryEdge() *jen.Statement {
	return jen.Id("rep").Dot("client").Dot("Query").Call().Dot("Where").Call(
...
func RepositoryInterfaceBuilder(pkg string) BuildFunc {
	return func(ctx context.Context, node *gen.Type, opts *BuildOptions) (*jen.File, error) {
		file := jen.NewFile(pkg)
		file.HeaderComment("Code generated by coconutc. DO NOT EDIT.")
		file.HeaderComment("https://github.com/syralon/coconutc")
