...
func ServiceBuilder(repoPkg, entityPkg string) BuildFunc {
	return func(ctx context.Context, node *gen.Type, opts *BuildOptions) (*jen.File, error) {
		file := jen.NewFile(strings.ToLower(node.Name) + "service")
...
func (b *serviceBuilder) structs() {
	...
	b.file.Type().Id(b.name).Struct(
		jen.Qual(b.ProtoPackage, fmt.Sprintf("Unsafe%sServer", b.name)),
...
func (b *serviceBuilder) listResponse(node *gen.Type) []jen.Code {
	return []jen.Code{
		jen.Id("Data").Op(":").Qual(pkgXSlices, "Trans").Call(
			jen.Id("data"),
			jen.Qual(b.entityPackage, fmt.Sprintf("%sToProto", node.Name)),
		).Op(","),
